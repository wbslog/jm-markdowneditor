...
> 📜 **라이선스**: 개인·기업 누구나 **무료로 사용**할 수 있습니다 (업무 사용 포함).
> 단, **판매·재판매·유료 배포·리브랜딩은 금지**됩니다. 자세한 내용은 [LICENSE](LICENSE) 참조.
...
| 기능 | 방법 |
|------|------|
| 파일 메뉴 | 상단 `📁 파일` 드롭다운에 열기/다른 이름 저장/모두 저장/모두 닫기 그룹화 |
| 파일 열기 | `📁 파일 → 📂 열기` 또는 `Ctrl+O` (여러 파일 = 탭으로 열림) |
| 저장 (원본 md에 저장) | 상단 `💾 저장` 버튼(🔍 검색 왼쪽) 또는 `Ctrl+S` — Confluence 문서는 Confluence에, 로컬 파일은 로컬에 저장 |
...
| 검색 | `🔍 검색` 버튼 또는 `Ctrl+F` — **프로그램 상단 중앙에 뜨는 검색 바**로 에디터/미리보기 동시 하이라이트·이동, **전체보기(에디터 전용/미리보기 전용) 상태에서도 사용 가능** |
...
| Confluence 연동 | 연동 완료 시 `🔗 Confluence 조회` 메뉴 노출 — 문서 목록 조회·열기·편집·저장(업데이트), `☁️ Confluence 업로드`로 새 페이지 생성 |
| 설명서 | `❓ 설명서` — 프로그램 사용법 / 마크다운 문법 30선 / 버전 기록 (모달, 검색 지원) |
| 언어 전환 | 우측 상단 `KOR / ENG` 버튼 — 메뉴·설명·안내 전체를 한/영으로 전환(설정 저장) |
| 파일 관리 | 디렉토리 패널에서 우클릭(열기·이름 변경·삭제·새 파일·새 폴더 — 생성 직후 바로 이름 변경), **폴더 삭제 가능**, 파일/폴더 드래그로 다른 폴더 이동 |
| 다중 선택 | 파일 다중 선택(드래그 러버밴드 · `Ctrl`/`⌘`+클릭 · `Shift`+범위) 후 우클릭 → **열기(N개) 동시 열기** / 삭제(N개). 폴더는 선택 제외 |
| 드래그 앤 드롭 | 탐색기(Windows)/Finder(macOS)에서 파일을 드롭하면 원본 경로로 열림(파일명·경로 표시, 저장 시 원본 업데이트+백업) |
| 파일 정보 | 에디터 상단에 현재 파일의 최초 생성일 / 최근 수정일 표시 |
| 파일명 표시 | 편집기 · 웹에디터 · 미리보기 상단에 현재 열린 파일명 표시(📄) — 이미 열린 파일은 중복 없이 해당 탭으로 이동 |
| 동기 스크롤 | 좌/우 어느 쪽을 스크롤해도 반대편이 비율 기준으로 동기 이동 |
| 파일 탭 | 탭 클릭 = 전환(디스크 변경 시 재로딩 확인), 드래그 = 순서 변경, ◀▶ = 한 탭씩 이동, ✕ = 닫기, **탭 우클릭 = 💾 저장 / 🔄 새로고침 / ✕ 닫기 메뉴** |
| 디렉토리 패널 | 폴더/`..` 한 번 클릭 = 이동, 파일 더블클릭 = 열기(숨김·모든 파일 표시), 경계선 드래그 = 너비 조절, 상단 **🔄 새로고침** 버튼으로 목록 갱신 |
...
앱을 시작하면 GitHub Releases에서 새 버전을 자동 확인합니다.

- 새 버전이 있으면 **안내창**이 뜨고, **다운로드 및 재시작**을 누르면 OS에 맞는 릴리스 파일을
  **기존 프로그램이 있는 폴더에 내려받은 뒤** 기존 앱을 종료하고 새 파일을 자동 실행합니다.
- 새 버전을 처음 실행하면 **추가/개선 내용**(릴리스에 작성한 메시지)이 안내창으로 표시됩니다.
  **다시 보지 않기**를 체크하면 이후 버전에서도 표시되지 않습니다.

> **릴리스 올릴 때 규칙** (자동 업데이트가 인식하려면):
> - 태그: `v1.7.0` 형식 (`v` + 버전)
> - 파일명: Windows는 `...exe`, macOS는 이름에 `mac`이 들어간 `...zip` (예: `jm-mdv-1.7.0.exe`, `jm-mdv-1.7.0-mac-arm64.zip`)
> - **릴리스 설명(body)** 에 적은 내용이 "새로운 기능" 안내창에 그대로 표시됩니다 (마크다운 지원)
...
1. **API 토큰 발급** — 아래 두 가지 방법 중 하나로 발급합니다. (스코프 없는 **클래식 토큰** 권장)

   **방법 A — 직접 링크(가장 빠름)**
   1. 브라우저에서 [id.atlassian.com/manage-profile/security/api-tokens](https://id.atlassian.com/manage-profile/security/api-tokens) 접속 후 Atlassian 계정으로 로그인
   2. **Create API token (API 토큰 만들기)** 버튼 클릭
      - "Create API token with scopes"가 아닌 **일반 Create API token**을 선택하세요 (스코프형은 권한 부족으로 403이 날 수 있음)
   3. **Label (라벨/토큰 이름)** 입력 → **Create (만들기)**
   4. 표시된 토큰 문자열을 **즉시 복사** — ⚠️ 창을 닫으면 다시 볼 수 없습니다
   5. 복사한 값을 앱 설정의 **토큰 값** 칸에 붙여넣기

   **방법 B — Confluence 화면에서 찾아가기**
   1. Confluence 우측 상단 **프로필 아이콘(내 사진)** 클릭
   2. **Manage account (계정 관리)** 클릭 → Atlassian 계정 페이지로 이동
   3. 상단 탭에서 **Security (보안)** 클릭
   4. **API tokens (API 토큰)** 항목의 **Create and manage API tokens (API 토큰 만들기 및 관리)** 클릭
   5. 이후는 방법 A의 2~5단계와 동일
...
| 버전 | 날짜 | 주요 내용 |
|------|------|-----------|
| **v1.21.0** | 2026-07-27 | **Home 키 — 커서가 있는 줄의 가장 왼쪽으로 이동** (소스 에디터·웹에디터·미리보기 공통) — 소스 에디터는 워드랩된 시각적 줄이 아닌 실제 줄의 첫 글자로, 미리보기는 가로 스크롤을 맨 왼쪽으로 |
| **v1.20.0** | 2026-07-24 | **검색 바를 프로그램 상단(중앙)으로 이동** — 에디터·미리보기 어느 화면에서도(전체보기 포함) 검색·하이라이트·이동이 동작하도록 개선 |
| **v1.19.0** | 2026-07-22 | **디렉토리 패널 🔄 새로고침 버튼** 추가, **파일 탭 우클릭 메뉴**(💾 저장 / 🔄 새로고침 / ✕ 닫기) 추가 — 특정 탭을 활성화하지 않고도 저장·디스크 재로딩·닫기 (Win/Mac 공통) |
| **v1.18.4** | 2026-07-16 | **긴급 핫픽스** — 버전 기록 문구에 이스케이프 안 된 백틱이 들어가 앱 스크립트 전체가 중단되던 문제 수정(v1.18.2·v1.18.3에서 파일 복원 불가·상단 버튼 전부 먹통). **v1.18.2/1.18.3 대신 반드시 이 버전 사용** |
| **v1.18.3** | 2026-07-16 | **중복 열기 방지 보강** — Confluence 문서 조회·로컬 파일 더블클릭 시 이미 열린 탭이 있으면 새로 열지 않고 **해당 탭으로 포커스 이동**(Win/Mac 공통). Confluence 페이지 id 문자열 비교로 통일, 이미 열린 Confluence 탭 전환 시 `new_file` 오호출 제거 |
| **v1.18.2** | 2026-07-16 | **macOS 온보딩(시작하기) 재표시 버그 수정** — 종료 시 상태 저장 2회 호출이 경쟁하며 `onboarded` 기록이 덮어써지던 문제(macOS 한정)를 단일 저장으로 해결, 사용 설명서(한/영) 라이선스 반영 |
| **v1.18.1** | 2026-07-16 | **📜 라이선스 추가** — 무료 사용(개인·기업) 허용, 판매·재판매·유료 배포 금지 (LICENSE 한/영) |
| **v1.18.0** | 2026-07-15 | **macOS 원클릭 설치/업데이트** — 터미널 한 줄(`easy_mac_setup.sh`)로 다운로드→빌드→설치→실행 자동화, 업데이트 창에 📋 명령 복사 버튼 |
| **v1.17.0** | 2026-07-15 | **macOS 업데이트 분기** — 자동 교체 대신 케이스별(릴리스/빌드) 안내 페이지 열기, Windows는 종전대로 자동 업데이트 |
| **v1.16.0** | 2026-07-15 | **파일 다중 선택**(드래그·`Ctrl`/`⌘`·`Shift`) 후 **열기(N개)/삭제(N개)**, **폴더 삭제 지원**, 웹에디터 세로 스크롤바 수정 |
| **v1.15.1** | 2026-07-15 | **설명서(도움말) 창 폭 +20%**(860→1032px, 상한 90→94vw) |
| **v1.15.0** | 2026-07-15 | **편집기·웹에디터·미리보기 상단에 파일명 표시**, **이미 열린 파일 중복 열기 방지 강화**(경로 구분자·대소문자 무시, 로컬·Confluence 공통) |
| **v1.14.0** | 2026-07-15 | 디렉토리 **새 폴더·생성 직후 이름 변경·폴더 드래그 이동·우클릭 메뉴 화면 이탈 방지**, **💾 저장 버튼 분리**, 🔗 Confluence 조회 / ☁️ Confluence 업로드 명칭, 웹에디터 스크롤 수정, 토큰 발급 툴팁, 설명서 현행화(Win/Mac 단축키) |
| **v1.13.0** | 2026-07-13 | 업데이트 **다운로드 프로그레스 바**, 설명서 상단 **🚀 업데이트 수동 확인** 버튼 |
| **v1.12.5** | 2026-07-13 | 위치 ◀ 화살표 **강조 + 느린 하트비트**(2.8초) 효과 |
| **v1.12.4** | 2026-07-13 | 위치 마커 디자인 변경 — 라인 제거, **우측 ◀ 화살표만 표시**(텍스트 가림 해결) |
| **v1.12.3** | 2026-07-13 | 위치 마커 **앵커 정렬**(제목/표/코드 기준) — 표 클릭 시 해당 행에 정확히 표시 |
| **v1.12.2** | 2026-07-13 | 위치 마커 **블록 매핑 정확도 개선·지속 표시**, 검색 매치 **좌우 중앙 포커싱**, 강조 하트비트 애니메이션 |
| **v1.12.1** | 2026-07-10 | 웹에디터: 명칭 변경(웹에디터), 색상표 열림 수정, **표 그리드(최대 10×10) 선택** 삽입 |
| **v1.12.0** | 2026-07-09 | **📍 편집 위치 → 프리뷰 마커** 표시(소스/웹 편집기 클릭 시 미리보기에 위치 강조) |
| **v1.11.1** | 2026-07-09 | 웹에디터 스크롤 동기화 수정, Confluence 자식 조회를 3경로 병합으로 누락 없이 표시 |
| **v1.11.0** | 2026-07-09 | 동기 스크롤 정확도 개선, Confluence 하위 페이지 표시 수정·목록 정리, 메뉴/버튼 폰트 +5%, 웹에디터 기능 대폭 확장 |
| **v1.10.0** | 2026-07-09 | **🖊 웹에디터(WYSIWYG)** 추가, 전체보기/컨플저장 버튼 디자인 개선, Confluence 목록 날짜·하위 표시 수정 |
| **v1.9.0** | 2026-07-09 | Confluence 조회를 **게시판형 목록**으로 개편(등록일·수정일 표시, 밝은 톤), 하위 있을 때만 펼치기 아이콘, 노드별 새로고침 |
| **v1.8.0** | 2026-07-09 | **👋 최초 실행 온보딩** — 첫 실행 시 대표 기능 5가지를 모달로 1회만 안내(닫으면 다시 안 뜸) |
| **v1.7.0** | 2026-07-09 | **🚀 자동 업데이트** — 시작 시 릴리스 확인, OS별 파일 다운로드 후 자동 재시작, 새 버전 첫 실행 시 변경 내역 안내(다시 보지 않기) |
...
### 🍎 macOS — 원클릭 설치 · 업데이트 (누구나 가능, 권장)

> ⚠️ **릴리스에는 macOS용 파일이 없습니다.** 맥은 직접 빌드해야 하지만,
> 아래 **한 줄**만 붙여넣으면 다운로드부터 빌드·설치·실행까지 전부 자동으로 됩니다.
> **IT 지식이 없어도 됩니다** — 복사, 붙여넣기, Enter가 전부입니다.

1. **터미널 열기**: 키보드에서 `⌘(Command) + Space` → `터미널` 입력 → Enter
2. 아래 한 줄을 **복사해서 터미널에 붙여넣고 Enter**:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/wbslog/jm-markdowneditor/main/easy_mac_setup.sh | bash
   ```
3. 끝! 몇 분 뒤 앱이 자동으로 실행되고, **응용 프로그램(Launchpad)에 `jm-mdv`** 가 설치됩니다.

- **최초 1회**, "명령어 도구(Command Line Tools) 설치" 창이 뜰 수 있습니다 → **[설치]** 클릭 후
  설치가 끝나면 **위의 한 줄을 한 번 더** 실행해 주세요. (Apple 공식 도구입니다)
- **업데이트도 똑같습니다** — 새 버전이 나오면 위 한 줄을 다시 실행하면 최신 버전으로 교체됩니다.
- 스크립트가 하는 일: 소스 다운로드(`~/jm-mdv-src`) → 가상환경에 패키지 설치(시스템은 건드리지 않음)
  → 앱 빌드 → `응용 프로그램` 폴더에 복사 → 보안 격리 해제(`xattr`) → 실행

> **실행이 안 될 때** — 터미널에서 직접 실행하면 실제 오류가 표시됩니다:
> ```bash
> /Applications/jm-mdv.app/Contents/MacOS/jm-mdv
> ```
> - `developer cannot be verified` / "손상됨" → `xattr -cr /Applications/jm-mdv.app` 후 재시도
> - 그 외 오류는 위 한 줄 명령을 다시 실행하면 처음부터 깨끗하게 다시 빌드합니다.

#### 🔄 macOS 업데이트 방법 (앱 내 안내)

macOS는 코드 서명/공증이 없어 **자동 교체를 하지 않습니다.** 새 버전이 있으면 앱의 **🚀 업데이트** 창에 아래 안내와 버튼이 표시됩니다.

- **간단(권장)**: 터미널에 위의 **원클릭 한 줄**을 다시 붙여넣기 → 자동으로 최신 버전 재빌드·교체
- **수동(개발자)**: `🛠 빌드 가이드(README) 열기` 버튼 → `git pull` → 가상환경/패키지 → `pyinstaller` 빌드

> Windows는 종전대로 **⬇️ 다운로드 및 재시작** 으로 자동 업데이트됩니다.
...
> 💡 **개발 지식이 없다면 이 섹션은 필요 없습니다** — 위의
> [🍎 macOS — 원클릭 설치 · 업데이트](#-macos--원클릭-설치--업데이트-누구나-가능-권장) 한 줄로 충분합니다.
> 아래는 빌드 과정을 직접 제어하고 싶은 개발자용 상세 절차입니다.
...
easy_mac_setup.sh  # macOS 원클릭 설치/업데이트 스크립트 (비개발자용)
LICENSE            # 라이선스 (무료 사용 가능 · 재판매 금지)
requirements.txt
```

## 📜 라이선스

본 소프트웨어는 **프리웨어**입니다. 자세한 조건은 [LICENSE](LICENSE) 파일(한/영)에 있습니다.

| 구분 | 내용 |
|------|------|
| ✅ 허용 | 개인·기업·기관의 **무료 사용**(업무·상업 환경 내부 사용 포함), 조직 내 배포, 개인적 소스 열람·수정, 무료·원본 그대로 공유 |
| ❌ 금지 | **판매·재판매·유료 배포·대여**, 재라이선스, 이름을 바꾼 배포(리브랜딩), 유료 제품·서비스에 포함, 저작권 표시 제거 |
| ⚠️ 보증 | 무보증(AS-IS) — 사용으로 인한 손해에 대해 책임지지 않음 |

Copyright (c) 2026 **Shin.DoYeon** · [jinroh77@gmail.com](mailto:jinroh77@gmail.com)
