...
Confluence Cloud 문서를 앱에서 바로 열고 편집·저장할 수 있습니다.

1. Atlassian 계정 → **보안 → API 토큰**에서 토큰을 발급합니다.
2. 앱 상단 **⚙️ 설정**을 열고 아래를 입력합니다.
   - **아틀란시안 계정(email)**: Atlassian 로그인 이메일
   - **토큰 이름 / 토큰 값**: 발급한 API 토큰
   - **나의 작업 폴더 또는 링크**: 작업할 스페이스 링크
     (예: `https://사이트.atlassian.net/wiki/spaces/KEY/overview`, 특정 페이지 링크도 가능)
3. **🔌 연동 테스트**로 인증을 확인한 뒤 **저장**합니다.
4. 연동이 완료되면 상단에 **🔗 Confluence** 메뉴가 나타납니다.
   - 메뉴에서 문서 목록을 조회하고, 클릭해 열어 편집한 뒤 `Ctrl+S`(저장)로 **업데이트**합니다.
   - 새 문서는 Confluence 메뉴의 **"현재 내용을 새 페이지로 저장"**으로 제목을 입력해 생성합니다.
