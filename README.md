...
#### 🔄 macOS 업데이트 방법 (앱 내 안내 · 케이스별)

macOS는 코드 서명/공증이 없어 **자동 교체를 하지 않습니다.** 새 버전이 있으면 앱의 **🚀 업데이트** 창에 아래 케이스별 안내와 **페이지 열기 버튼**이 표시됩니다.

- **경우 A — 릴리스 앱(.app/zip)으로 사용 중**: `📦 릴리스 페이지 열기` → 최신 `*-mac.zip` 다운로드 → 기존 `.app` 교체 → `xattr -cr jm-mdv.app` 후 실행.
- **경우 B — 소스로 실행/빌드(개발자)**: `🛠 빌드 가이드(README) 열기` → `git pull` → 가상환경/패키지 설치 → `python3 app.py` 실행 또는 `pyinstaller` 로 `.app` 빌드.
