...
#   3. 결과물: dist/jm-mdv.app  (응용 프로그램 폴더로 복사해서 사용)
#      ※ 파일명에 버전을 넣지 않는다 - 버전이 바뀌어도 파일 연결이 유지되도록.
set -e
cd "$(dirname "$0")"

python3 -m pip install --upgrade pywebview markdown pymdown-extensions pygments pyinstaller

# app.py 의 APP_VERSION 은 안내 문구용으로만 읽는다 (파일명에는 넣지 않는다)
...
echo ""
echo "===== 빌드 완료: dist/jm-mdv.app  (버전 $VER) ====="
echo "처음 실행 시 '확인되지 않은 개발자' 경고가 나오면:"
echo "  Finder에서 우클릭 → 열기, 또는: xattr -cr dist/jm-mdv.app"
